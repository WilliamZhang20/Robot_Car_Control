...
MapMemoryNode::MapMemoryNode() : Node("map_memory"), map_memory_(robot::MapMemoryCore(this->get_logger())) {
  ...
  odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("/odom", 10, std::bind(&MapMemoryNode::odomCallback, this, std::placeholders::_1));
...
void MapMemoryNode::updateMap() {
  ...
  if (global_map.has_value()) {
      ...
      // If no new map generated, republish the last map if we have one
      if (map_memory_.hasGlobalMap()) {
