...
TebOptimalPlanner::TebOptimalPlanner(const rclcpp::Logger& logger) : logger_(logger), optimized_(false) {
  ...
  // prepare optimizer - use dynamic block solver for mixed vertex/edge dimensions
...
bool TebOptimalPlanner::buildGraph(double weight_multiplier) {
  ...
  RCLCPP_DEBUG(logger_, "Creating time vertices");
...
bool TebOptimalPlanner::optimizeTEB(int iterations_innerloop, int iterations_outerloop, bool compute_cost_afterwards,
  ...
  RCLCPP_DEBUG(logger_, "optimizeTEB called with %d inner, %d outer iterations", iterations_innerloop, iterations_outerloop);
  
  if (cfg_.optim.optimization_activate == false) {
    RCLCPP_DEBUG(logger_, "Optimization deactivated in config");
    return false;
  }
  
  if (!current_odom_ || !current_path_) {
    RCLCPP_DEBUG(logger_, "Missing data - odom: %s, path: %s", 
                 current_odom_ ? "available" : "missing",
                 current_path_ ? "available" : "missing");
    return false;
  }
  
  RCLCPP_DEBUG(logger_, "Path has %zu poses, TEB has %zu poses", 
               current_path_->poses.size(), teb_.size());
